...
class BaseSymbolic(BaseEstimator, metaclass=ABCMeta):
    ...
    def fit(self, X, y, sample_weight=None):
        ...
        for gen in range(prior_generations, self.generations):
            ...
            """
                Change Parallel to ray. Don't use n_jobs, n_programs, starts, add cpu or memory
                1 Need change fitness function wrap is False
                2 when X is very large, use obj_ref = ray.put(y) and ray.get(obj_ref)
            """
            seeds = random_state.randint(MAX_INT, size=self.population_size)

            parents_ref = ray.put(parents)

            future_ids = [_parallel_evolve.remote(1,
                                                  parents_ref,
                                                  x_ref,
                                                  y_ref,
                                                  sample_weight,
                                                  [seeds[i]],
                                                  params) for i in range(self.population_size)]
            population = ray.get(future_ids)
        ...
        if isinstance(self, SymbolicTransformerX):
            # all program
            programs_flat_list = []
            for sublist in self._programs:
                for item in sublist:
                    if item:
                        programs_flat_list.append(item)
            fitness_flat_list = [program.raw_fitness_ for program in programs_flat_list]
            if self._metric.greater_is_better:
                hall_of_fame_all_gen = np.array(fitness_flat_list).argsort()[::-1]
            else:
                hall_of_fame_all_gen = np.array(fitness_flat_list).argsort()
            self._best_programs = []
            _best_programs_str = []
            return_components = min(self.n_components, len(hall_of_fame_all_gen))
            for i in hall_of_fame_all_gen:
                if str(programs_flat_list[i]) not in _best_programs_str:
                    self._best_programs.append(programs_flat_list[i])
                    _best_programs_str.append(str(programs_flat_list[i]))
                    if len(self._best_programs) >= return_components:
                        break
            return self
...
class SymbolicTransformerX(SymbolicTransformer, TransformerMixin):
    def __init__(self,
                 population_size=1000,
                 hall_of_fame=100,
                 n_components=10,
                 ...
                 verbose=0,
                 random_state=None):
        super(SymbolicTransformerX, self).__init__(
            population_size=population_size,
            hall_of_fame=hall_of_fame,
            n_components=n_components,
            generations=generations,
