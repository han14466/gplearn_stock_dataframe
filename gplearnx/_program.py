...
class _Program(object):
    ...
    def build_program(self, random_state):
        ...
        if hasattr(function, 'is_ts') and function.is_ts:
            function = refactor_ts_function(function, random_state)
    ...
    def execute(self, X):
        ...
        for node in self.program:
            ...
            while len(apply_stack[-1]) == apply_stack[-1][0].arity + 1:
                ...
                terminals = [np.repeat(t, X.shape[0]) if isinstance(t, float) and isinstance(X, np.ndarray)
                             else pd.Series(np.repeat(t, X.shape[0]), index=X.index) if isinstance(t, float)
                                                                                        and isinstance(X, pd.DataFrame)
                else X[:, t] if isinstance(t, int) and isinstance(X, np.ndarray)
                else X.iloc[:, t] if isinstance(t, int) and isinstance(X, pd.DataFrame)
    ...
    def raw_fitness(self, X, y, sample_weight):
        ...
        raw_fitness = self.metric(y, y_pred, sample_weight, func_expression=self.__str__())
        return raw_fitness
    ...
    def point_mutation(self, random_state):
        ...
        for node in mutate:
            if isinstance(program[node], _Function):
                ...
                if hasattr(replacement, 'is_ts') and replacement.is_ts:
                    replacement = refactor_ts_function(replacement, random_state)
...
def refactor_ts_function(function, random_state):
    """
    Parameters
    ----------
    function: _Function
    ...
    """
    win = random_state.randint(len(function.days))
    win = function.days[win]
    function_ = copy(function)
    function_.set_win(win)
    return function_
